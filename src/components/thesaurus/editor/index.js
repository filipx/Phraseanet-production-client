...
//		for(src0=srcElement; src0 && (!src0.tagName); src0=src0.parentNode)
        ...
        if (src0.className === 'hslider') {
            var x;
            var wslider = parseInt(src0.style.width, 10);
            var posmax = parseInt(src0.parentNode.style.width, 10) - wslider;
            var min = 0, max = posmax;
            if ((x = src0.getAttribute('min')) !== '')
                min = parseInt(x, 10);
            if ((x = src0.getAttribute('max')) !== '')
                max = parseInt(x, 10);
            var pos = parseInt(src0.style.left, 10);
        ...
        if (src0.className === 'hsplitter') {
            src0.setAttribute('otop', parseInt(src0.style.top, 10));
            src0.setAttribute('ozindex', parseInt(src0.style.zIndex, 10));
            src0.style.zIndex = 999;
            gui.splitting = src0;
            return (false);
        }
        if (src0.className === 'vsplitter') {
            src0.setAttribute('oleft', parseInt(src0.style.left, 10));
            src0.setAttribute('ozindex', parseInt(src0.style.zIndex, 10));
            src0.style.zIndex = 999;
            gui.splitting = src0;
            return (false);
        ...
        var xmouse = parseInt(evt.clientX, 10);
        var ymouse = parseInt(evt.clientY, 10);
    ...
    function _ww_evt_mmov_doc(gui, evt) {
        ...
        dragObj.X = xmouse = parseInt(evt.clientX, 10);
        dragObj.Y = ymouse = parseInt(evt.clientY, 10);
//window.status = xmouse + '-' + ymouse;

        under_ballon = (evt.target) ? evt.target : evt.srcElement;
        if (under_ballon.id) {
            timer_ballon = setTimeout(() => {
                return timeout_ballon(under_ballon.id, xmouse, ymouse)
            }, 300);
        }

        xmouse = parseInt(evt.clientX, 10);
        ymouse = parseInt(evt.clientY, 10);
        if (dragObj.Src) {
            ...
            if (z !== 99) {
                ...
                for (i = 0; i < this.twin.length; i++) {
                    ...
                    if (o2 = document.getElementById(this.varname + 'w' + i)) {
                        z2 = parseInt(o2.style.zIndex, 10);
                        if (z2 > z && z2 !== 99) {
                            z2--;
...
// alert('recucer');
        if (o = document.getElementById(this.varname + 'w' + idx)) {
            if (this.twin[idx].reduced) {
                ...
                this.twin[idx].oldheight = parseInt(o.style.height, 10);
...
//	alert('tith=' + tith);
        tv = false;
        win_w = parseInt(this.properties.width, 10);
        win_h = parseInt(this.properties.height, 10);
        o_h = win_h - tith - 2;
        if (o_h < 0)
            o_h = 0;
        ...
        if (o = document.getElementById(this.varname + 'r' + this.idx)) {
            t = 'top:' + (parseInt(this.properties.height, 10) - 18) + 'px; left:' + (parseInt(this.properties.width, 10) - 18) + 'px;';
            o.style.cssText = t;
        }
    }

    function _ww_repaint() {
        w = parseInt(this.gui.twin[this.idx].properties.width, 10);
        h = parseInt(this.gui.twin[this.idx].properties.height, 10);
    ...
    function _ww_sizeto(w, h) {
        ...
        if (this.gui.twin[this.idx].onresized)
            (this.gui.twin[this.idx].onresized)(parseInt(this.gui.twin[this.idx].properties.top, 10)
                , parseInt(this.gui.twin[this.idx].properties.left, 10), w, h);
    }
    ...
    function _ww_getClientSize(view) {
        ...
        if (o = document.getElementById(this.varname + view + this.idx)) {
            return ({w: (parseInt(o.style.width, 10) - 2), h: (parseInt(o.style.height, 10) - 2)});
        }
        return (null);
    }
    ...
    function _ww_autoMove() {
        ...
        if (o = document.getElementById(this.varname + 'w' + this.idx).style) {
            ...
            twin = parseInt(o.top, 10);
            lwin = parseInt(o.left, 10);
            wwin = parseInt(o.width, 10);
            hwin = parseInt(o.height, 10);
            bwin = twin + hwin;
            if (lwin > wgui - wwin) {
                lwin = wgui - wwin;
                moved = true;
...
//	this.mdwn_win = ww_mdwn_win;
        ...
        for (p in this.properties) {
            if (properties[p]) {
                if (p === 'height') {
                    if (parseInt(properties[p], 10) < 60)
                        properties[p] = '60px';
                }
                this.properties[p] = properties[p];
            }
...
//		s += "	<td><div id=\""+varname+"t"+idx+"\" class=ww_winTitle>" + properties.title + "</div></td>";
        ...
        if (properties.width)
            style += 'width' + ':' + (parseInt(this.properties['width'], 10) - 2) + 'px;';
        if (properties.heigth)
            style += 'heigth' + ':' + (parseInt(this.properties['heigth'], 10) - 20) + 'px;';
...
// retourne la fenetre au premier plan (hors palettes 'ontop')
    ...
    function _ww_openCenterWindow(zurl, w, h, name, params) {
        ...
        p.status = 'no';
        p.resizable = 'no';
        for (let cc in params) {
            if (p[params[cc].n])
                p[params[cc].n] = params[cc].v;
        }
        let t = ((document.getElementById(this.desktop).clientHeight - h) / 2) + self.screenTop;
        let l = ((document.getElementById(this.desktop).clientWidth - w) / 2) + self.screenLeft;
        if (t < 10){
            t = 10;
        }
        if (l < 10){
        ...
        let s = 'top=' + t + ', left=' + l + ', width=' + w + ', height=' + h;
        for (let cc in p){
            s += ',' + cc + '=' + p[cc];
        }
    ...
    function _ww_runAsMenu(event, backparm) {
        ...
        if (typeof(event.which) !== 'undefined'){
            button = this.gui.firefoxButtons['b' + event.which];
        } else{
            // safari aussi ???
            button = event.button;
        }
        let xmouse = parseInt(event.clientX, 10);
        let ymouse = parseInt(event.clientY, 10);
    ...
    function _ww_setAsMenu(id, callback) {
        let o = this.document.getElementById(id);
        o.ismenu = 1;
        o.gui = this;
...
export default thesaurusEditor;
